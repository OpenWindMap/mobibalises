 ...
 * @author pedro.m
 */
public final class AtterrissageFfvlContentHandler extends FfvlContentHandler
{
  ...
  private static final String ATTERRISSAGE_ID_ATT   = "identifiant";
  ...
  public void endElement(final String uri, final String localName, final String qName) throws SAXException
  {
    ...
    {
      atterrissage.nom = currentString;
    }
    else if (ACCES_TAG.equals(finalName))
    {
      if (!Utils.isStringVide(currentString))
      {
        atterrissage.infos.put(FfvlSpotProvider.Info.ACCES.getKey(), currentString);
      }
    }
  ...
  public void startElement(final String uri, final String localName, final String qName, final Attributes atts) throws SAXException
  {
    ...
    {
      atterrissage = new FfvlSpot();
      atterrissage.type = TypeSpot.ATTERRISSAGE;
      atterrissage.idSite = atts.getValue(ATTERRISSAGE_ID_ATT);
      spots.add(atterrissage);
    }
    else if (COORD_TAG.equals(finalName))
    {
      try
      {
        atterrissage.latitude = Utils.parseDouble(atts.getValue(COORD_LATITUDE_ATT));
        atterrissage.longitude = Utils.parseDouble(atts.getValue(COORD_LONGITUDE_ATT));
      }
    ...
    {
      atterrissage.addPratique(getPratique(atts.getValue(PRATIQUE_VALUE_ATT)));
    }
    else if (ALTITUDE_TAG.equals(finalName))
    {
      try
      {
        atterrissage.altitude = Utils.parseInteger(atts.getValue(ALTITUDE_VALUE_ATT));
      }
      catch (final NumberFormatException nfe)
      {
        //Nothing
    ...
    {
      getOrientations(atts.getValue(ORIENTATION_VALUE_ATT), atterrissage.orientations);
    }
    else if (STRUCTURE_TAG.equals(finalName))
    {
      atterrissage.idStructure = atts.getValue(STRUCTURE_VALUE_ATT);
    }
    else if (ID_SITE_TAG.equals(finalName))
    {
      atterrissage.setId(atts.getValue(ID_SITE_VALUE_ATT));
    }
